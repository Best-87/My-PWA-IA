...
export const TelegramLogin: React.FC<TelegramLoginProps> = ({ onAuth }) => {
    ...
    useEffect(() => {
        // Ejecutar sólo una vez, protegiendo contra el React StrictMode o re-renders rápidos.
        ...
        console.info(`[Telegram Auth] Inicializando conexión (Única instancia) para: @${botName}`);
        ...
        // Sobrescribir el callback de manera segura globalmente
        (window as any).onTelegramAuth = (user: any) => {
            console.log("✅ [Telegram Auth] Éxito. Datos de Autenticación recibidos:", user);
            onAuth(user);
        };

        containerRef.current.appendChild(script);

        // Intencionalmente EVITAMOS borrar el onTelegramAuth global o vaciar el innerHTML
        // en el cleanup, porque el usuario puede estar interactuando y un micro-render
        // lo mataría dejando el proceso a medias.
        return () => {
            // Se mantiene vivo mientras el componente Profile exista
        };
    }, [onAuth]);
